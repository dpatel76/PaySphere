...
const MappingsDocumentationPage: React.FC = () => {
  ...
  const paginatedData = useMemo(() => {
    ...
    return documentation.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  }, [documentation, page, rowsPerPage]);
        ...
        <Paper>
          <TableContainer sx={{ maxHeight: 600 }}>
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: '#f5f5f5' }}>Standard Field</TableCell>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: '#f5f5f5' }}>Description</TableCell>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: '#f5f5f5' }}>Standard Type</TableCell>
                  ...
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: ZONE_COLORS.gold }}>Gold Column</TableCell>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: ZONE_COLORS.gold }}>Gold Type</TableCell>
                  <TableCell sx={{ fontWeight: 'bold', bgcolor: ZONE_COLORS.gold }}>Purpose</TableCell>
              ...
              <TableBody>
                {paginatedData.map((row, idx) => (
                  <TableRow
                    key={`${row.standard_field_id || idx}-${row.silver_mapping_id || idx}`}
                    hover
                    sx={{
                  ...
                  >
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                        {row.is_mapped_to_silver ? (
                          <MappedIcon sx={{ fontSize: 16, color: 'success.main' }} />
                        ) : (
                          <UnmappedIcon sx={{ fontSize: 16, color: 'error.main' }} />
                        )}
                        <Typography variant="body2" fontWeight={row.standard_field_mandatory ? 600 : 400}>
                          {row.standard_field_name || '-'}
                        </Typography>
                        {row.standard_field_mandatory && (
                          <Chip label="REQ" size="small" color="error" sx={{ height: 16, fontSize: 10 }} />
                        )}
                      </Box>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Typography variant="body2">{row.standard_field_data_type || '-'}</Typography>
                    </TableCell>
                    <TableCell>
                      <Tooltip title={row.standard_field_allowed_values || ''}>
                        <Typography variant="body2" noWrap sx={{ maxWidth: 100 }}>
                          {row.standard_field_allowed_values || '-'}
                        </Typography>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <Tooltip title={row.standard_field_path || ''}>
                        <Typography variant="body2" noWrap sx={{ maxWidth: 150, fontFamily: 'monospace', fontSize: 11 }}>
                          {row.standard_field_path || '-'}
                        </Typography>
                      </Tooltip>
                    </TableCell>
                    <TableCell>
                      <Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: 11 }}>
                        {row.bronze_table || 'raw_payment_messages'}
                      </Typography>
                    </TableCell>
                    <TableCell>
                      <Typography
                    ...
                    <TableCell>
                      <Typography variant="body2">{row.silver_data_type || '-'}</Typography>
                    </TableCell>
                    <TableCell>
                      <Typography
                        variant="body2"
                        sx={{
                          fontFamily: 'monospace',
                          fontSize: 11,
                          color: row.gold_table ? 'text.primary' : 'text.disabled',
                        }}
                      >
                        {row.gold_table?.replace('gold.', '') || '-'}
                      </Typography>
                    </TableCell>
                    <TableCell>
                      <Typography
                        ...
                        sx={{
                          fontFamily: 'monospace',
                          fontSize: 11,
                          color: row.gold_column ? 'text.primary' : 'text.disabled',
                        }}
                      >
                        {row.gold_column || '-'}
                      </Typography>
                    ...
                    <TableCell>
                      <Typography variant="body2">{row.gold_data_type || '-'}</Typography>
                    </TableCell>
                    <TableCell>
                      {row.gold_purpose_code ? (
                        <Chip label={row.gold_purpose_code} size="small" variant="outlined" />
                      ) : row.gold_entity_role ? (
                        <Chip label={row.gold_entity_role} size="small" color="primary" variant="outlined" />
                      ) : (
                        '-'
                      )}
                    </TableCell>
