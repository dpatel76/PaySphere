"""Thailand PromptPay Extractor.

ISO 20022 INHERITANCE HIERARCHY:
    PromptPay has two internal formats:
    1. ISO 8583: ATM/POS transactions (internal network)
    2. JSON: QR Code and API transactions

    For cross-border (ASEAN) and certain domestic flows, PromptPay uses ISO 20022.
    This extractor provides conversion from ISO 8583 → ISO 20022 format.

    BaseISO20022Parser
        └── Pacs008Parser (FI to FI Customer Credit Transfer)
            ├── PromptPayPacs008Parser (ISO 20022 cross-border)
            ├── Pacs002Parser (Payment Status Report)
            │   └── PromptPayPacs002Parser
            └── Pacs004Parser (Payment Return)
                └── PromptPayPacs004Parser

SUPPORTED MESSAGE TYPES:
    Native:
    - ISO 8583: ATM/POS card network format
    - JSON: QR Code, API transactions

    Converted to ISO 20022:
    - pacs.008: Cross-border Credit Transfer
    - pacs.002: Payment Status Report
    - pacs.004: Payment Return

PROMPTPAY-SPECIFIC ELEMENTS:
    - Proxy types: MOBILE, TAXID (13 digits), EWALLET, BILLER
    - THB currency (Thai Baht)
    - Bank of Thailand (BOT) 3-digit bank codes
    - Thai tax ID (13 digits for organizations)
    - QR Code standard EMVCo format

CLEARING SYSTEM:
    - THPROMPTPAY (Thailand PromptPay - instant payment)
    - THBAHTNET (Thailand BAHTNET - RTGS)
    - THITMX (Thailand Interbank Transaction Management)

ISO 8583 to ISO 20022 MAPPING:
    The conversion layer maps ISO 8583 data elements to pacs.008 fields:
    - DE2 (PAN) → DbtrAcct/Id/Othr/Id
    - DE3 (Processing Code) → PmtTpInf/LclInstrm/Cd
    - DE4 (Amount) → IntrBkSttlmAmt
    - DE32 (Acquiring Institution) → DbtrAgt/FinInstnId/ClrSysMmbId
    - DE41 (Card Acceptor Terminal) → PmtId/InstrId
    - DE43 (Card Acceptor Name/Location) → Cdtr/Nm
    - DE49 (Transaction Currency) → IntrBkSttlmAmt@Ccy
    - DE102 (Account ID 1) → CdtrAcct/Id/Othr/Id

DATABASE TABLES:
    - Bronze: bronze.raw_payment_messages
    - Silver: silver.stg_promptpay
    - Gold: Semantic tables via DynamicGoldMapper
"""

from typing import Dict, Any, List, Optional
from datetime import datetime
import json
import logging
import re

from ..base import (
    BaseExtractor,
    ExtractorRegistry,
    GoldEntities,
    PartyData,
    AccountData,
    FinancialInstitutionData,
)

logger = logging.getLogger(__name__)

# Import ISO 20022 base classes for inheritance
try:
    from ..iso20022 import (
        Pacs008Parser, Pacs008Extractor,
        Pacs002Parser, Pacs002Extractor,
        Pacs004Parser, Pacs004Extractor,
    )
    ISO20022_BASE_AVAILABLE = True
except ImportError:
    ISO20022_BASE_AVAILABLE = False
    logger.warning("ISO 20022 base classes not available - PromptPay will use standalone implementation")


# =============================================================================
# PROMPTPAY CONSTANTS
# =============================================================================

CLEARING_SYSTEM = "THPROMPTPAY"  # PromptPay instant payment
BAHTNET_CLEARING_SYSTEM = "THBAHTNET"  # BAHTNET RTGS
ITMX_CLEARING_SYSTEM = "THITMX"  # ITMX interbank
DEFAULT_CURRENCY = "THB"
DEFAULT_COUNTRY = "TH"


# =============================================================================
# ISO 8583 TO ISO 20022 CONVERTER
# =============================================================================

class ISO8583ToISO20022Converter:
    """Converts ISO 8583 message fields to ISO 20022 pacs.008 structure.

    ISO 8583 is the standard for ATM/POS card network transactions.
    This converter maps ISO 8583 data elements to ISO 20022 pacs.008 fields
    for cross-border interoperability.

    ISO 8583 Data Elements used:
        DE2: Primary Account Number (PAN)
        DE3: Processing Code (determines transaction type)
        DE4: Transaction Amount
        DE7: Transmission Date/Time
        DE11: System Trace Audit Number (STAN)
        DE12: Local Transaction Time
        DE13: Local Transaction Date
        DE32: Acquiring Institution ID (bank code)
        DE37: Retrieval Reference Number
        DE41: Card Acceptor Terminal ID
        DE43: Card Acceptor Name/Location
        DE49: Transaction Currency Code
        DE102: Account Identification 1 (receiver)
        DE103: Account Identification 2 (additional)
    """

    # ISO 8583 Processing Code to ISO 20022 Local Instrument mapping
    PROCESSING_CODE_MAP = {
        '00': 'INST',   # Purchase / Credit Transfer
        '01': 'INST',   # Cash Withdrawal (QR)
        '20': 'INST',   # Refund
        '30': 'INST',   # Available Balance Inquiry
        '31': 'INST',   # Balance Inquiry
        '40': 'INST',   # Transfer
    }

    def convert(self, iso8583_msg: Dict[str, Any]) -> Dict[str, Any]:
        """Convert ISO 8583 message to ISO 20022-like structure.

        Args:
            iso8583_msg: Dict with ISO 8583 data elements keyed by DE number

        Returns:
            Dict with ISO 20022 pacs.008 field names
        """
        result = {
            'messageType': 'PROMPTPAY',
            'isPromptPay': True,
            'sourceFormat': 'ISO8583',
            'clearingSystem': CLEARING_SYSTEM,
        }

        # Message identification
        result['transactionId'] = iso8583_msg.get('DE37')  # Retrieval Reference
        result['endToEndId'] = f"PP-{iso8583_msg.get('DE11', '')}"  # STAN
        result['instructionId'] = iso8583_msg.get('DE41')  # Terminal ID

        # Date/Time
        de7 = iso8583_msg.get('DE7', '')  # MMDDhhmmss format
        de12 = iso8583_msg.get('DE12', '')  # hhmmss
        de13 = iso8583_msg.get('DE13', '')  # MMDD
        if de12 and de13:
            result['creationDateTime'] = f"2026-{de13[:2]}-{de13[2:4]}T{de12[:2]}:{de12[2:4]}:{de12[4:6]}+07:00"

        # Amount
        de4 = iso8583_msg.get('DE4', '0')
        try:
            # ISO 8583 amounts are typically in minor units (satang for THB)
            result['amount'] = float(de4) / 100.0
        except ValueError:
            result['amount'] = 0.0

        # Currency
        de49 = iso8583_msg.get('DE49', '764')  # Thailand = 764
        result['currency'] = 'THB' if de49 == '764' else DEFAULT_CURRENCY

        # Processing type
        de3 = iso8583_msg.get('DE3', '00')[:2]
        result['localInstrument'] = self.PROCESSING_CODE_MAP.get(de3, 'INST')

        # Payer (from PAN and acquiring institution)
        result['payerAccount'] = iso8583_msg.get('DE2')  # PAN
        result['payerBankCode'] = iso8583_msg.get('DE32')  # Acquiring Institution

        # Payee (from account identification and card acceptor)
        result['payeeAccount'] = iso8583_msg.get('DE102')
        result['payeeName'] = iso8583_msg.get('DE43')  # Card Acceptor Name/Location

        # Additional ISO 8583 fields for extension
        result['stan'] = iso8583_msg.get('DE11')  # System Trace Audit Number
        result['terminalId'] = iso8583_msg.get('DE41')
        result['merchantCategoryCode'] = iso8583_msg.get('DE18')
        result['responseCode'] = iso8583_msg.get('DE39')
        result['approvalCode'] = iso8583_msg.get('DE38')

        return result


# =============================================================================
# PROMPTPAY ISO 20022 PARSERS (for cross-border flows)
# =============================================================================

_Pacs008Base = Pacs008Parser if ISO20022_BASE_AVAILABLE else object
_Pacs002Base = Pacs002Parser if ISO20022_BASE_AVAILABLE else object
_Pacs004Base = Pacs004Parser if ISO20022_BASE_AVAILABLE else object


class PromptPayPacs008Parser(_Pacs008Base):
    """PromptPay pacs.008 parser - Cross-border Credit Transfer."""

    CLEARING_SYSTEM = CLEARING_SYSTEM
    DEFAULT_CURRENCY = DEFAULT_CURRENCY
    MESSAGE_TYPE = "PROMPTPAY_pacs008"

    def __init__(self):
        if ISO20022_BASE_AVAILABLE:
            super().__init__()

    def parse(self, raw_content: str) -> Dict[str, Any]:
        # Handle dict input (pre-parsed)
        if isinstance(raw_content, dict):
            return raw_content

        # Handle JSON input
        if isinstance(raw_content, str) and raw_content.strip().startswith('{'):
            try:
                return json.loads(raw_content)
            except json.JSONDecodeError:
                pass

        # Parse as ISO 20022 XML (cross-border)
        if ISO20022_BASE_AVAILABLE:
            result = super().parse(raw_content)
        else:
            result = {'messageType': 'PROMPTPAY_pacs008'}

        result['isPromptPay'] = True
        result['clearingSystem'] = self.CLEARING_SYSTEM
        result['defaultCurrency'] = self.DEFAULT_CURRENCY
        return result


class PromptPayPacs002Parser(_Pacs002Base):
    """PromptPay pacs.002 parser - Payment Status Report."""

    CLEARING_SYSTEM = CLEARING_SYSTEM
    DEFAULT_CURRENCY = DEFAULT_CURRENCY
    MESSAGE_TYPE = "PROMPTPAY_pacs002"

    def __init__(self):
        if ISO20022_BASE_AVAILABLE:
            super().__init__()

    def parse(self, raw_content: str) -> Dict[str, Any]:
        if isinstance(raw_content, dict):
            return raw_content

        if ISO20022_BASE_AVAILABLE:
            result = super().parse(raw_content)
        else:
            result = {'messageType': 'PROMPTPAY_pacs002'}

        result['isPromptPay'] = True
        result['clearingSystem'] = self.CLEARING_SYSTEM
        return result


class PromptPayPacs004Parser(_Pacs004Base):
    """PromptPay pacs.004 parser - Payment Return."""

    CLEARING_SYSTEM = CLEARING_SYSTEM
    DEFAULT_CURRENCY = DEFAULT_CURRENCY
    MESSAGE_TYPE = "PROMPTPAY_pacs004"

    def __init__(self):
        if ISO20022_BASE_AVAILABLE:
            super().__init__()

    def parse(self, raw_content: str) -> Dict[str, Any]:
        if isinstance(raw_content, dict):
            return raw_content

        if ISO20022_BASE_AVAILABLE:
            result = super().parse(raw_content)
        else:
            result = {'messageType': 'PROMPTPAY_pacs004'}

        result['isPromptPay'] = True
        result['clearingSystem'] = self.CLEARING_SYSTEM
        return result


# =============================================================================
# PROMPTPAY JSON PARSER (QR Code and API format)
# =============================================================================

class PromptPayJsonParser:
    """Parser for PromptPay JSON format messages (QR Code and API).

    Handles the flat JSON structure:
    {
        "transactionId": "PROMPTPAY-E2E-20260106-001",
        "endToEndId": "E2E-PP-20260106-001",
        "creationDateTime": "2026-01-06T14:15:00+07:00",
        "amount": 125000.00,
        "currency": "THB",
        "payerProxyType": "MOBILE",
        "payerProxyValue": "0891234567",
        "payerName": "Bangkok Trading Corporation Ltd",
        "payerTaxId": "0105556000001",
        "payerBankCode": "002",
        "payerBankBic": "BKKBTHBKXXX",
        "payerAccount": "1234567890",
        "payerAddress": { ... },
        "payeeName": "Chiang Mai Export Services Co Ltd",
        "payeeAccount": "9876543210",
        "payeeAddress": { ... },
        "qrCodeData": "00020101021229370016...",
        "remittanceInfo": { "structured": {...}, "unstructured": "..." }
    }
    """

    def parse(self, raw_content: str) -> Dict[str, Any]:
        """Parse PromptPay JSON message into structured dict."""
        result = {
            'messageType': 'PROMPTPAY',
            'isPromptPay': True,
            'sourceFormat': 'JSON',
            'clearingSystem': CLEARING_SYSTEM,
        }

        # Handle dict input (pre-parsed)
        if isinstance(raw_content, dict):
            data = raw_content
        else:
            # Parse JSON string
            if raw_content.strip().startswith('{'):
                try:
                    data = json.loads(raw_content)
                except json.JSONDecodeError as e:
                    logger.warning(f"Failed to parse PromptPay JSON: {e}")
                    return result
            else:
                logger.warning("PromptPay content is not JSON")
                return result

        # Core fields
        result['transactionId'] = data.get('transactionId')
        result['endToEndId'] = data.get('endToEndId')
        result['creationDateTime'] = data.get('creationDateTime')
        result['settlementDate'] = data.get('settlementDate')
        result['acceptanceDateTime'] = data.get('acceptanceDateTime')

        # Amount
        result['amount'] = data.get('amount')
        result['currency'] = data.get('currency') or DEFAULT_CURRENCY
        result['chargeBearer'] = data.get('chargeBearer')

        # Payment type
        result['paymentType'] = data.get('paymentType')
        result['localInstrument'] = data.get('localInstrument')

        # Payer information
        result['payerProxyType'] = data.get('payerProxyType')  # MOBILE, TAXID, EWALLET, BILLER
        result['payerProxyValue'] = data.get('payerProxyValue')
        result['payerName'] = data.get('payerName')
        result['payerTaxId'] = data.get('payerTaxId')
        result['payerBankCode'] = data.get('payerBankCode')
        result['payerBankName'] = data.get('payerBankName')
        result['payerBankBic'] = data.get('payerBankBic')
        result['payerAccount'] = data.get('payerAccount')
        result['payerAccountType'] = data.get('payerAccountType')

        payer_addr = data.get('payerAddress', {})
        result['payerStreetName'] = payer_addr.get('streetName')
        result['payerBuildingNumber'] = payer_addr.get('buildingNumber')
        result['payerFloor'] = payer_addr.get('floor')
        result['payerPostalCode'] = payer_addr.get('postalCode')
        result['payerCity'] = payer_addr.get('city')
        result['payerDistrict'] = payer_addr.get('district')
        result['payerCountry'] = payer_addr.get('country') or DEFAULT_COUNTRY

        payer_contact = data.get('payerContact', {})
        result['payerContactName'] = payer_contact.get('name')
        result['payerContactPhone'] = payer_contact.get('phone')
        result['payerContactEmail'] = payer_contact.get('email')

        # Payee information
        result['payeeProxyType'] = data.get('payeeProxyType')
        result['payeeProxyValue'] = data.get('payeeProxyValue')
        result['payeeName'] = data.get('payeeName')
        result['payeeTaxId'] = data.get('payeeTaxId')
        result['payeeBankCode'] = data.get('payeeBankCode')
        result['payeeBankName'] = data.get('payeeBankName')
        result['payeeBankBic'] = data.get('payeeBankBic')
        result['payeeAccount'] = data.get('payeeAccount')
        result['payeeAccountType'] = data.get('payeeAccountType')

        payee_addr = data.get('payeeAddress', {})
        result['payeeStreetName'] = payee_addr.get('streetName')
        result['payeeBuildingNumber'] = payee_addr.get('buildingNumber')
        result['payeeFloor'] = payee_addr.get('floor')
        result['payeePostalCode'] = payee_addr.get('postalCode')
        result['payeeCity'] = payee_addr.get('city')
        result['payeeDistrict'] = payee_addr.get('district')
        result['payeeCountry'] = payee_addr.get('country') or DEFAULT_COUNTRY

        payee_contact = data.get('payeeContact', {})
        result['payeeContactName'] = payee_contact.get('name')
        result['payeeContactPhone'] = payee_contact.get('phone')
        result['payeeContactEmail'] = payee_contact.get('email')

        # QR Code data
        result['qrCodeData'] = data.get('qrCodeData')

        # Remittance information
        remit = data.get('remittanceInfo', {})
        if isinstance(remit, dict):
            structured = remit.get('structured', {})
            result['invoiceNumber'] = structured.get('invoiceNumber')
            result['invoiceDate'] = structured.get('invoiceDate')
            result['dueAmount'] = structured.get('dueAmount')
            result['remittanceUnstructured'] = remit.get('unstructured')
        else:
            result['remittanceUnstructured'] = str(remit) if remit else None

        # Purpose
        result['purposeCode'] = data.get('purposeCode')
        result['categoryPurpose'] = data.get('categoryPurpose')

        return result


# =============================================================================
# UNIFIED PROMPTPAY PARSER (auto-detects format)
# =============================================================================

class PromptPayUnifiedParser:
    """Unified PromptPay parser that auto-detects format (JSON, ISO 8583, ISO 20022).

    Format detection:
    - JSON starting with '{' → PromptPayJsonParser
    - ISO 8583 (dict with DE keys) → ISO8583ToISO20022Converter
    - XML containing ISO 20022 root elements → ISO 20022 parsers
    """

    ROOT_TO_PARSER = {
        'FIToFICstmrCdtTrf': 'pacs008',
        'FIToFIPmtStsRpt': 'pacs002',
        'PmtRtr': 'pacs004',
    }

    def __init__(self):
        self.pacs008_parser = PromptPayPacs008Parser()
        self.pacs002_parser = PromptPayPacs002Parser()
        self.pacs004_parser = PromptPayPacs004Parser()
        self.json_parser = PromptPayJsonParser()
        self.iso8583_converter = ISO8583ToISO20022Converter()

    def parse(self, raw_content: str) -> Dict[str, Any]:
        """Parse PromptPay message, auto-detecting format."""
        # Handle dict input
        if isinstance(raw_content, dict):
            # Check if it's ISO 8583 format (has DE keys)
            if any(k.startswith('DE') for k in raw_content.keys()):
                return self.iso8583_converter.convert(raw_content)
            # Otherwise assume JSON format
            return self.json_parser.parse(raw_content)

        content = raw_content.strip() if isinstance(raw_content, str) else ''

        # JSON format
        if content.startswith('{'):
            parsed = json.loads(content)
            # Check if it's ISO 8583 format
            if any(k.startswith('DE') for k in parsed.keys()):
                return self.iso8583_converter.convert(parsed)
            return self.json_parser.parse(content)

        # XML format = ISO 20022
        msg_type = self._detect_message_type(content)

        parser_map = {
            'pacs008': self.pacs008_parser,
            'pacs002': self.pacs002_parser,
            'pacs004': self.pacs004_parser,
        }

        parser = parser_map.get(msg_type, self.pacs008_parser)
        return parser.parse(content)

    def _detect_message_type(self, xml_content: str) -> str:
        """Detect ISO 20022 message type from XML root element."""
        for root_elem, msg_type in self.ROOT_TO_PARSER.items():
            if root_elem in xml_content:
                return msg_type
        return 'pacs008'  # Default


# =============================================================================
# PROMPTPAY EXTRACTOR
# =============================================================================

class PromptPayExtractor(BaseExtractor):
    """Extractor for Thailand PromptPay instant payment messages.

    Supports three formats:
    1. JSON: QR Code and API transactions (most common)
    2. ISO 8583: ATM/POS card network format (converted to ISO 20022)
    3. ISO 20022 XML: Cross-border ASEAN transactions

    The ISO 8583 → ISO 20022 converter maps data elements to pacs.008 fields
    for unified processing in the CDM pipeline.
    """

    MESSAGE_TYPE = "PROMPTPAY"
    SILVER_TABLE = "stg_promptpay"
    DEFAULT_CURRENCY = DEFAULT_CURRENCY
    CLEARING_SYSTEM = CLEARING_SYSTEM

    def __init__(self):
        """Initialize extractor with unified parser."""
        super().__init__()
        self.unified_parser = PromptPayUnifiedParser()
        self.parser = self.unified_parser

    # =========================================================================
    # BRONZE EXTRACTION
    # =========================================================================

    def extract_bronze(self, raw_content: Dict[str, Any], batch_id: str) -> Dict[str, Any]:
        """Extract Bronze layer record from raw PromptPay content."""
        # Parse if string
        if isinstance(raw_content, str):
            parsed = self.parser.parse(raw_content)
        else:
            parsed = raw_content

        msg_id = parsed.get('transactionId', '') or parsed.get('endToEndId', '')
        return {
            'raw_id': self.generate_raw_id(msg_id),
            'message_type': self.MESSAGE_TYPE,
            'raw_content': json.dumps(raw_content) if isinstance(raw_content, dict) else raw_content,
            'batch_id': batch_id,
        }

    # =========================================================================
    # SILVER EXTRACTION
    # =========================================================================

    def extract_silver(
        self,
        msg_content: Dict[str, Any],
        raw_id: str,
        stg_id: str,
        batch_id: str
    ) -> Dict[str, Any]:
        """Extract all Silver layer fields from PromptPay message.

        Handles all three formats (JSON, ISO 8583, ISO 20022) through
        the unified parser.
        """
        trunc = self.trunc

        # Parse if needed
        if isinstance(msg_content, str):
            parsed = self.parser.parse(msg_content)
        elif not msg_content.get('isPromptPay') and not msg_content.get('isISO20022'):
            parsed = self.parser.parse(msg_content)
        else:
            parsed = msg_content

        # Handle field name variations across formats
        payer_name = (
            parsed.get('payerName') or
            parsed.get('debtorName')
        )
        payer_account = (
            parsed.get('payerAccount') or
            parsed.get('debtorAccountOther') or
            parsed.get('debtorAccountIban')
        )
        payee_name = (
            parsed.get('payeeName') or
            parsed.get('creditorName')
        )
        payee_account = (
            parsed.get('payeeAccount') or
            parsed.get('creditorAccountOther') or
            parsed.get('creditorAccountIban')
        )

        return {
            'stg_id': stg_id,
            'raw_id': raw_id,
            '_batch_id': batch_id,

            # Message Type
            'message_type': 'PROMPTPAY',
            'transaction_id': trunc(
                parsed.get('transactionId') or
                parsed.get('transactionId'),
                35
            ),
            'creation_date_time': parsed.get('creationDateTime'),

            # Amount
            'amount': parsed.get('amount'),
            'currency': parsed.get('currency') or DEFAULT_CURRENCY,

            # Payer (Debtor)
            'payer_proxy_type': trunc(parsed.get('payerProxyType'), 10),
            'payer_proxy_value': trunc(parsed.get('payerProxyValue'), 50),
            'payer_name': trunc(payer_name, 140),
            'payer_bank_code': trunc(
                parsed.get('payerBankCode') or
                parsed.get('debtorAgentMemberId') or
                parsed.get('instructingAgentMemberId'),
                11
            ),
            'payer_account': trunc(payer_account, 34),

            # Payee (Creditor)
            'payee_proxy_type': trunc(parsed.get('payeeProxyType'), 10),
            'payee_proxy_value': trunc(parsed.get('payeeProxyValue'), 50),
            'payee_name': trunc(payee_name, 140),
            'payee_bank_code': trunc(
                parsed.get('payeeBankCode') or
                parsed.get('creditorAgentMemberId') or
                parsed.get('instructedAgentMemberId'),
                11
            ),
            'payee_account': trunc(payee_account, 34),

            # Payment Details
            'end_to_end_id': trunc(
                parsed.get('endToEndId') or
                parsed.get('endToEndId'),
                35
            ),
            'qr_code_data': parsed.get('qrCodeData'),
            'remittance_info': parsed.get('remittanceUnstructured'),

            # Additional parsed fields
            'settlement_date': parsed.get('settlementDate') or parsed.get('interbankSettlementDate'),
            'acceptance_date_time': parsed.get('acceptanceDateTime'),
            'charge_bearer': trunc(parsed.get('chargeBearer'), 10),
            'payment_type': trunc(
                parsed.get('paymentType') or
                parsed.get('localInstrumentCode'),
                10
            ),
            'local_instrument': trunc(parsed.get('localInstrument'), 10),
            'payer_tax_id': trunc(parsed.get('payerTaxId'), 20),
            'payer_bank_name': trunc(
                parsed.get('payerBankName') or
                parsed.get('debtorAgentName'),
                140
            ),
            'payer_bank_bic': trunc(
                parsed.get('payerBankBic') or
                parsed.get('debtorAgentBic'),
                11
            ),
            'payer_account_type': trunc(parsed.get('payerAccountType'), 10),
            'payee_tax_id': trunc(parsed.get('payeeTaxId'), 20),
            'payee_bank_name': trunc(
                parsed.get('payeeBankName') or
                parsed.get('creditorAgentName'),
                140
            ),
            'payee_bank_bic': trunc(
                parsed.get('payeeBankBic') or
                parsed.get('creditorAgentBic'),
                11
            ),
            'payee_account_type': trunc(parsed.get('payeeAccountType'), 10),
            'purpose_code': trunc(
                parsed.get('purposeCode') or
                parsed.get('categoryPurposeCode'),
                10
            ),
            'category_purpose': trunc(parsed.get('categoryPurpose'), 10),
        }

    def get_silver_columns(self) -> List[str]:
        """Return ordered list of Silver table columns for INSERT.

        NOTE: Column names MUST match silver.stg_promptpay table exactly.
        """
        return [
            'stg_id', 'raw_id', '_batch_id',
            'message_type', 'message_id', 'transaction_id', 'creation_date_time',
            'amount', 'currency',
            'payer_proxy_type', 'payer_proxy_value', 'payer_name',
            'payer_bank_code', 'payer_account',
            'payee_proxy_type', 'payee_proxy_value', 'payee_name',
            'payee_bank_code', 'payee_account',
            'end_to_end_id', 'qr_code_data', 'remittance_info',
        ]

    def get_silver_values(self, silver_record: Dict[str, Any]) -> tuple:
        """Return ordered tuple of values for Silver table INSERT."""
        columns = self.get_silver_columns()
        return tuple(silver_record.get(col) for col in columns)

    # =========================================================================
    # GOLD ENTITY EXTRACTION
    # =========================================================================

    def extract_gold_entities(
        self,
        silver_data: Dict[str, Any],
        stg_id: str,
        batch_id: str
    ) -> GoldEntities:
        """Extract Gold layer entities from PromptPay Silver record.

        Args:
            silver_data: Dict with Silver table columns (snake_case field names)
            stg_id: Silver staging ID
            batch_id: Batch identifier
        """
        entities = GoldEntities()

        # Determine party type based on proxy type or tax ID presence
        payer_proxy_type = silver_data.get('payer_proxy_type') or ''
        payee_proxy_type = silver_data.get('payee_proxy_type') or ''

        # Payer Party (Debtor) - uses Silver column names
        if silver_data.get('payer_name'):
            # TAXID proxy type indicates organization
            party_type = 'ORGANIZATION' if payer_proxy_type == 'TAXID' or silver_data.get('payer_tax_id') else 'INDIVIDUAL'
            entities.parties.append(PartyData(
                name=silver_data.get('payer_name'),
                role="DEBTOR",
                party_type=party_type,
                identification_type='TAXID' if silver_data.get('payer_tax_id') else payer_proxy_type if payer_proxy_type else None,
                identification_number=silver_data.get('payer_tax_id') or silver_data.get('payer_proxy_value'),
                country=DEFAULT_COUNTRY,
            ))

        # Payee Party (Creditor)
        if silver_data.get('payee_name'):
            party_type = 'ORGANIZATION' if payee_proxy_type == 'TAXID' or silver_data.get('payee_tax_id') else 'INDIVIDUAL'
            entities.parties.append(PartyData(
                name=silver_data.get('payee_name'),
                role="CREDITOR",
                party_type=party_type,
                identification_type='TAXID' if silver_data.get('payee_tax_id') else payee_proxy_type if payee_proxy_type else None,
                identification_number=silver_data.get('payee_tax_id') or silver_data.get('payee_proxy_value'),
                country=DEFAULT_COUNTRY,
            ))

        # Payer Account (or Proxy)
        if silver_data.get('payer_account') or silver_data.get('payer_proxy_value'):
            entities.accounts.append(AccountData(
                account_number=silver_data.get('payer_account') or silver_data.get('payer_proxy_value'),
                role="DEBTOR",
                account_type=silver_data.get('payer_account_type') or 'CACC',
                currency=silver_data.get('currency') or DEFAULT_CURRENCY,
            ))

        # Payee Account (or Proxy)
        if silver_data.get('payee_account') or silver_data.get('payee_proxy_value'):
            entities.accounts.append(AccountData(
                account_number=silver_data.get('payee_account') or silver_data.get('payee_proxy_value'),
                role="CREDITOR",
                account_type=silver_data.get('payee_account_type') or 'CACC',
                currency=silver_data.get('currency') or DEFAULT_CURRENCY,
            ))

        # Payer Bank
        if silver_data.get('payer_bank_code') or silver_data.get('payer_bank_bic'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="DEBTOR_AGENT",
                name=silver_data.get('payer_bank_name'),
                bic=silver_data.get('payer_bank_bic'),
                clearing_code=silver_data.get('payer_bank_code'),
                clearing_system=CLEARING_SYSTEM,
                country=DEFAULT_COUNTRY,
            ))

        # Payee Bank
        if silver_data.get('payee_bank_code') or silver_data.get('payee_bank_bic'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="CREDITOR_AGENT",
                name=silver_data.get('payee_bank_name'),
                bic=silver_data.get('payee_bank_bic'),
                clearing_code=silver_data.get('payee_bank_code'),
                clearing_system=CLEARING_SYSTEM,
                country=DEFAULT_COUNTRY,
            ))

        return entities


# =============================================================================
# REGISTER EXTRACTORS
# =============================================================================

ExtractorRegistry.register('PROMPTPAY', PromptPayExtractor())
ExtractorRegistry.register('promptpay', PromptPayExtractor())
ExtractorRegistry.register('PromptPay', PromptPayExtractor())
ExtractorRegistry.register('PROMPTPAY_ISO8583', PromptPayExtractor())  # ISO 8583 format alias

# Message type specific aliases
ExtractorRegistry.register('PROMPTPAY_pacs008', PromptPayExtractor())
ExtractorRegistry.register('PROMPTPAY_pacs002', PromptPayExtractor())
ExtractorRegistry.register('PROMPTPAY_pacs004', PromptPayExtractor())
