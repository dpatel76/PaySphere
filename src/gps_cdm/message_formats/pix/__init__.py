...
class PixExtractor(BaseExtractor):
    ...
    def extract_gold_entities(
        ...
        """Extract Gold layer entities from PIX message."""
        ...
        if msg_content.get('payerName'):
            entities.parties.append(PartyData(
                ...
                role="DEBTOR",
                party_type='INDIVIDUAL' if len(msg_content.get('payerCpfCnpj', '')) == 11 else 'ORGANIZATION',
                identification_type='CPF' if len(msg_content.get('payerCpfCnpj', '')) == 11 else 'CNPJ',
                identification_number=msg_content.get('payerCpfCnpj'),
                country='BR',
            ))
        ...
        if msg_content.get('payeeName'):
            entities.parties.append(PartyData(
                ...
                role="CREDITOR",
                party_type='INDIVIDUAL' if len(msg_content.get('payeeCpfCnpj', '')) == 11 else 'ORGANIZATION',
                identification_type='CPF' if len(msg_content.get('payeeCpfCnpj', '')) == 11 else 'CNPJ',
                identification_number=msg_content.get('payeeCpfCnpj'),
                country='BR',
            ))

        # Payer Account
        if msg_content.get('payerAccount') or msg_content.get('payerPixKey'):
            entities.accounts.append(AccountData(
                account_number=msg_content.get('payerAccount') or msg_content.get('payerPixKey'),
                role="DEBTOR",
                account_type=msg_content.get('payerAccountType') or 'CACC',
                currency='BRL',
            ))

        # Payee Account
        if msg_content.get('payeeAccount') or msg_content.get('payeePixKey'):
            entities.accounts.append(AccountData(
                account_number=msg_content.get('payeeAccount') or msg_content.get('payeePixKey'),
                role="CREDITOR",
                account_type=msg_content.get('payeeAccountType') or 'CACC',
                currency='BRL',
            ))

        # Payer Bank (by ISPB)
        if msg_content.get('payerIspb'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="DEBTOR_AGENT",
                clearing_code=msg_content.get('payerIspb'),
                clearing_system='BRISPB',  # Brazil ISPB
                country='BR',
            ))

        # Payee Bank (by ISPB)
        if msg_content.get('payeeIspb'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="CREDITOR_AGENT",
                clearing_code=msg_content.get('payeeIspb'),
