...
class InstaPayExtractor(BaseExtractor):
    ...
    def extract_gold_entities(
        ...
        """Extract Gold layer entities from InstaPay message."""
        entities = GoldEntities()

        # Sender Party (Debtor)
        if msg_content.get('senderName'):
            entities.parties.append(PartyData(
                name=msg_content.get('senderName'),
                role="DEBTOR",
                party_type='UNKNOWN',
                country='PH',
            ))
        ...
        if msg_content.get('receiverName'):
            entities.parties.append(PartyData(
                name=msg_content.get('receiverName'),
                role="CREDITOR",
                party_type='UNKNOWN',
                country='PH',
            ))
        ...
        # Sender Account
        if msg_content.get('senderAccount'):
            entities.accounts.append(AccountData(
                account_number=msg_content.get('senderAccount'),
                role="DEBTOR",
                account_type='CACC',
                currency=msg_content.get('currency') or 'PHP',
            ))
        ...
        # Receiver Account
        if msg_content.get('receiverAccount'):
            entities.accounts.append(AccountData(
                account_number=msg_content.get('receiverAccount'),
                role="CREDITOR",
                account_type='CACC',
                currency=msg_content.get('currency') or 'PHP',
            ))
        ...
        # Sender Bank
        if msg_content.get('senderBankCode'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="DEBTOR_AGENT",
                clearing_code=msg_content.get('senderBankCode'),
                clearing_system='PHINSTAPAY',
                country='PH',
            ))

        # Receiver Bank
        if msg_content.get('receiverBankCode'):
            entities.financial_institutions.append(FinancialInstitutionData(
                role="CREDITOR_AGENT",
                clearing_code=msg_content.get('receiverBankCode'),
                clearing_system='PHINSTAPAY',
                country='PH',
            ))
